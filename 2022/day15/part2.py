...
for i, row in enumerate([list(merge_ranges(invalid[x])) for x in range(0, 4000001)]):
    if len(row) > 1:
        print(i, row)
        print((row[0][1] * 4000000) + i)
    '''
    # set([y for x in [range(4), range(7)] for y in x if y >= 0 if y <= 20]))
    #print(row)
    list_of_ranges = [range(z[0], z[1]) for z in row]
    s = set([y for x in list_of_ranges for y in x if y >= 0 if y <= 4000000])
    #print(len(s))
    if len(s) <= 4000000:
        print(i)
    '''
